...
**Every row tells you why it matched.** A hit reached through aliases is rendered as what you typed — `Alpha/Delta/Echo` — with the real path, `Alpha/Bravo/Charlie.md`, underneath it. A row matched by real names alone shows one line, because there is nothing to explain.
