...
// plugin's demo vault does its own startup setup. Here it installs the one plugin Alias Quick
// Switcher cannot run without, then opens the landing note.
...
// Alias Quick Switcher declares it as a dependency and loads nothing until it is there.
const DEPENDENCY_PLUGIN_ID = 'advanced-metadata-cache';
...
export async function invoke(app: App): Promise<void> {
  ...
  // changes nothing. Installing it changes nothing about what the switcher matches either: its Titles
  // module is off until you turn it on, which `04 Extra names` does with a button.
